...
(function () {
    ...
    fetch("data.json")
      ...
      .then((data) => {
        if (data.themeSounds) {
          ...
          if (soundSettings.enabled && soundSettings.sounds) {
            // Lazy load the audio files in the background after the page loads
            setTimeout(() => {
              Object.keys(soundSettings.sounds).forEach((key) => {
                const audio = new Audio();
                audio.preload = "auto"; // Download in the background
                audio.src = soundSettings.sounds[key];
                audio.load();
                preloadedAudio[key] = audio;
              });
            }, 3000); // 3 seconds delay to not block initial page load resources
          }
        }
      })
  ...
  function playThemeSound(themeKey) {
    ...
    // Play new audio
    if (preloadedAudio[themeKey]) {
      currentAudio = preloadedAudio[themeKey];
      currentAudio.currentTime = 0;
    } else {
